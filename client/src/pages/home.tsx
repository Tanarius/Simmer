...
import {
  ChefHat, ShoppingCart, Calendar, Package, Sparkles,
  ChevronRight, AlertCircle, CheckCircle2, Flame
} from "lucide-react";
...
function QuickStat({ icon, label, value, href, accent }: { icon: React.ReactNode; label: string; value: string | number; href: string; accent?: string }) {
  const [, setLocation] = useLocation();
  return (
    <button
      onClick={() => setLocation(href)}
      className="flex-1 min-w-[120px] flex flex-col items-start gap-1 p-4 rounded-2xl border border-border bg-card hover:bg-accent/40 transition-colors text-left"
    >
  ...
  return (
    <div className="space-y-2">
      {todaySlots.map(({ type, recipe }) => recipe && (
        <div key={type} className={cn("rounded-2xl p-3 flex items-center gap-3", getCuisineGradient(recipe.cuisine ?? "other"))}>
          <span className="text-2xl">{getFoodEmoji(recipe.name, recipe.cuisine)}</span>
          ...
          <Button
            size="sm"
            variant="secondary"
            className="shrink-0 h-7 px-2 text-xs bg-white/20 hover:bg-white/30 text-white border-0"
            onClick={() => setLocation("/")}
          >
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        <QuickStat icon={<ChefHat className="h-4 w-4" />} label="recipes" value={recipes?.length ?? 0} href="/" accent="text-emerald-500" />
        <QuickStat icon={<Calendar className="h-4 w-4" />} label="meals this week" value={plannedCount} href="/planner" accent="text-violet-500" />
        ...
        {activity.length === 0 ? (
          ...
          <div className="space-y-1">
            {activity.slice(0, 15).map(entry => (
              <div key={entry.id} className="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-accent/30 transition-colors">
                {/* Avatar */}
                ...
                {/* Time */}
                <span className="text-xs text-muted-foreground shrink-0">{compactTime(entry.createdAt)}</span>
              </div>
            ))}
          </div>
        )}
