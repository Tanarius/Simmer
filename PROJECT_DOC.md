...
Simmer is a household meal planning web app. A small group (roommates, family, couple) shares one account space: a recipe library, a weekly planner, a pantry tracker, and a shopping list. An AI copilot helps them plan meals around their taste preferences, pantry contents, and constraints.

**Live deployment**: Railway (PostgreSQL via Neon)
**Repository**: https://github.com/Tanarius/Simmer
...
### Core
- **Recipe Library** â€” CRUD recipes with ingredients (JSON), instructions (JSON), cuisine tag, cook/prep time, servings, image URL. Favorite toggle. URL import (JSON-LD schema.org extraction + HTML fallback).
- **Weekly Planner** â€” Drag-and-drop meal planning grid (Monâ€“Sun Ã— breakfast/lunch/dinner). Breakfast row toggle (localStorage). Day notes. Meal attribution (who added what slot). Emoji reactions per slot.
- **Shopping List** â€” Auto-generated from planned recipes. Ingredients grouped by store category, deduped, quantities combined.
- **Pantry** â€” Pantry staples tracker. Activity feed (recipe added, deleted, plan updated, meal added).
- **Household System** â€” Users belong to a household. Invite via shareable link (`/#/join/<code>`). 16-char invite tokens. Regenerate to invalidate old links. Member list. Leave household. Rename household.

### AI Features (powered by Anthropic Claude)
- **Kitchen Copilot** â€” Conversational assistant in a right drawer/bottom sheet. Multi-step: picks meal type â†’ cuisine â†’ vibe â†’ protein (optional) â†’ searches Spoonacular + Edamam â†’ returns 3 recipes â†’ can save directly to library.
- **Pantry Chef** â€” Suggests recipes from current pantry contents.
- **Weekly Plan AI** â€” Generates a full week plan using Anthropic based on taste profile.
- **Recipe Cleaner** â€” Normalizes imported recipes (strips ads, fixes units, reformats instructions).
- **Auto-tagging** â€” Tags recipes as crockpot / grilled / air-fryer / quick / slow-cook / make-ahead / freezer-friendly / one-pot at save time.
...
- Onboarding flow: cooking mode â†’ dietary needs â†’ cuisine preferences â†’ dietary restrictions.
...
meal_reactions  weekStart, slotKey, userId, emoji â€” UNIQUE(week, slot, user)
...
| 8 | Register accepts any input | Username: 3â€“30 chars, `[a-zA-Z0-9_.-]` only; password: 8+ chars |
...
| 2 | No email verification | Low | Users can register with no email â€” no account recovery |
| 3 | `SESSION_SECRET` defaults to hardcoded string in dev | Low | Acceptable for local dev; production blocks on missing value |
| 4 | In-memory AI cache grows unbounded | Low | `server/utils/cache.ts` has no max-size eviction â€” memory leak under heavy load |
| 5 | No HTTPS enforcement | Low | Railway handles TLS termination; no HTTPâ†’HTTPS redirect in app itself |
...
1. **Freemium SaaS** â€” $4â€“8/mo premium tier per household. Low friction, align with "household" unit (one payment for 2â€“4 users).
2. **One-time purchase + credits** â€” Pay once for app access, buy AI credit bundles. Lower recurring friction.
3. **Affiliate / recipe partnerships** â€” Link recipe sources; Spoonacular + Edamam have affiliate programs.
4. **Grocery integration** â€” Instacart/Kroger affiliate links on shopping list items (significant revenue potential).
...
### Organic / Content
- **SEO recipe pages**: Each saved recipe as a public shareable page (`/recipes/:id/public`) â€” long-tail search traffic for recipe queries.
- **Meal plan templates**: Public shareable week plans ("college budget plan", "30-min weeknight meals") â€” Pinterest-friendly, viral potential.
- **TikTok/Reels**: Weekly plan time-lapses, pantry-to-table recipe reveals.

### Referral / Viral
- **Invite mechanic already built**: Invite link flow is designed for household sharing â€” this is a natural K-factor loop (one user invites roommates).
- **"Planned with Simmer" share card**: After planning a week, generate a visual card to share. Brand exposure.
...
- **Reddit**: Organic + paid in r/SimmerSunday (900k members), r/Frugal, r/college.

### Partnerships
- **Grocery delivery** (Instacart, Walmart+): Shopping list â†’ one-click order. Revenue share potential.
- **Nutrition apps** (MyFitnessPal): Bi-directional sync â€” meal plan â†’ calorie tracking.
...
### Phase 1 â€” Launch Ready (blockers)
- [x] Household-scoped data isolation â€” DONE
- [x] PostgreSQL session store (connect-pg-simple) â€” DONE
- [ ] Premium tier payment (Stripe) â€” gate the AI calls properly
- [ ] Email/password reset flow (or "magic link")

### Phase 2 â€” Growth Features
...
```
ANTHROPIC_API_KEY       # Claude AI â€” required
DATABASE_URL            # Neon PostgreSQL â€” required
SESSION_SECRET          # Express session â€” required in production
SPOONACULAR_API_KEY     # 365k recipes, free tier
EDAMAM_APP_ID           # 2.3M recipes â€” PAID ($29/mo Recipe Search API)
