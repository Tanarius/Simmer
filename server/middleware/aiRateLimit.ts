...
export const TEST_TIER_DAILY_LIMIT = 50;
// applies to: test accounts (subscriptionTier = 'test')

export const COPILOT_FREE_TIER_DAILY_LIMIT = 20;
// applies to: /api/ai/copilot/chat only

export const COPILOT_TEST_TIER_DAILY_LIMIT = 50;
...
export async function copilotRateLimit(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (usage.subscriptionTier !== 'premium' && usage.copilotCallsToday >= copilotLimit) {
      return res.status(429).json({
    ...
    await storage.incrementCopilotCalls(userId);
    next();
  } catch (error) {
