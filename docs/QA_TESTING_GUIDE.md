...
- Tests marked 🤖 can be automated via Playwright (Tier 4 on the test roadmap)
- Tests marked 🔐 are security-critical — never skip them
...
|---|------|----------|----------|
| R-01 | Register with valid username (6+ chars, alphanumeric) and password (8+ chars) | Account created, redirect to `/onboarding` | P0 🤖 |
| R-02 | Register without email field | Error: "A valid email address is required" | P0 🔐 |
| R-03 | Register with invalid email (no TLD) | Rejected | P0 🔐 |
| R-04 | Register with username < 3 chars | Rejected | P1 |
| R-05 | Register with username containing spaces | Rejected (only `[a-zA-Z0-9_.-]` allowed) | P1 🔐 |
| R-06 | Register with password < 8 chars | Rejected | P1 |
| R-07 | Register with duplicate username | Error: "Username already taken" | P1 |
| R-08 | Register with duplicate email | Silent success (email collision handled gracefully) | P1 🔐 |
| R-09 | Attempt 6+ registrations from same IP in 1 hour | HTTP 429 from `registerRateLimit` | P1 🔐 |

### 1.2 Login
| # | Test | Expected | Priority |
|---|------|----------|----------|
| L-01 | Login with correct credentials | Redirect to `/` (recipe library) | P0 🤖 |
| L-02 | Login with wrong password | 401, error message shown | P0 |
| L-03 | Login with non-existent username | 401, generic error (no user enumeration) | P0 🔐 |
| L-04 | 11 failed login attempts within 15 minutes | HTTP 429 from `authRateLimit` | P1 🔐 |
| L-05 | Legacy bcrypt migration: first login with old plaintext-stored password | Login succeeds, password re-hashed, subsequent login uses bcrypt | P1 🔐 |
| L-06 | After bcrypt upgrade, open session still works | No re-login required mid-session | P1 |

### 1.3 Logout
| # | Test | Expected | Priority |
|---|------|----------|----------|
| LO-01 | Click "Sign out" in sidebar | Session destroyed, redirect to `/auth` | P0 🤖 |
| LO-02 | After logout, navigate to `/` directly | Redirect to `/auth` | P0 🔐 |
| LO-03 | After logout, all API calls return 401 | No data accessible | P0 🔐 |

### 1.4 Password Reset
| # | Test | Expected | Priority |
|---|------|----------|----------|
| PR-01 | Click "Forgot password?" on login page | Reset request form shown | P0 |
| PR-02 | Submit valid email for existing account | "If an account exists, you'll receive an email" (vague by design) | P0 🔐 |
| PR-03 | Submit email for non-existent account | Same vague success message (no email enumeration) | P0 🔐 |
...
| PR-06 | Try to use reset link after it expires (>15 min) | "Reset link is invalid or has expired" | P1 🔐 |
| PR-07 | Use reset link twice | Second attempt rejected (token cleared on use) | P1 🔐 |
| PR-08 | 6+ reset attempts from same IP in 15 min | HTTP 429 from `forgotRateLimit` | P1 🔐 |

### 1.5 Password Change (Authenticated)
| # | Test | Expected | Priority |
|---|------|----------|----------|
| PC-01 | Change password with correct current password | Success, new password works | P1 |
| PC-02 | Change password with wrong current password | Rejected | P1 🔐 |
| PC-03 | Change password with new password < 8 chars | Rejected | P1 |
| PC-04 | Pre-bcrypt account attempts password change without re-login | 400: "Please log out and log back in first" | P1 🔐 |
| PC-05 | Unauthenticated PATCH to `/api/auth/password` | 401 | P0 🔐 |
...
| # | Test | Expected | Priority |
|---|------|----------|----------|
| RF-01 | Load recipe library with 0 saved recipes | Empty state with "Add your first recipe" CTA | P0 🤖 |
| RF-02 | Search by partial recipe name (case-insensitive) | Matching recipes shown, non-matching hidden | P0 🤖 |
...
| # | Test | Expected | Priority |
|---|------|----------|----------|
| RA-01 | Submit form with name, cuisine, meal type filled | Recipe created, appears in library | P0 🤖 |
...
| RA-07 | Submit unauthenticated | 401 | P0 🔐 |

### 2.3 URL Import
| # | Test | Expected | Priority |
|---|------|----------|----------|
| UI-01 | Import valid AllRecipes URL | Form fields auto-populate; name, ingredients, instructions filled | P0 🤖 |
...
| UI-05 | Import `http://localhost/` | Blocked by SSRF validator | P0 🔐 |
| UI-06 | Import `http://169.254.169.254/` (AWS metadata) | Blocked | P0 🔐 |
| UI-07 | Import `http://10.0.0.1/` | Blocked | P0 🔐 |
| UI-08 | Import `file:///etc/passwd` | Blocked | P0 🔐 |
...
| # | Test | Expected | Priority |
|---|------|----------|----------|
| RV-01 | Click recipe card → view dialog opens | All recipe fields visible | P0 🤖 |
...
| RV-07 | Delete recipe that's in a current week plan | Plan slot not broken — recipe name still shows from meta | P2 |
...
| SI-07 | Submit social import unauthenticated | 401 | P0 🔐 |
...
| # | Test | Expected | Priority |
|---|------|----------|----------|
| WP-01 | Load planner for current week | Grid shows Mon–Sun × lunch/dinner | P0 🤖 |
...
| WP-04 | Select a recipe for a slot | Recipe appears in slot with name + cook time | P0 🤖 |
...
| # | Test | Expected | Priority |
|---|------|----------|----------|
| MA-01 | Alice adds a slot — attribution shows "Alice" | Attribution stored in `meal_meta` | P2 |
...
| AI-WP-03 | Free tier user who has used 5 AI calls today | 429 with upgrade prompt | P1 🔐 |
| AI-WP-04 | Existing plan slots are preserved when AI fills gaps | Only empty slots filled | P2 |

### 3.5 Emoji Reactions
| # | Test | Expected | Priority |
|---|------|----------|----------|
| ER-01 | React to a meal slot with 👍 | Reaction saved, visible to other household members | P1 |
...
| ER-04 | 🔐 User B cannot see Household A's reactions | GET reactions returns empty for wrong household | P0 🔐 |
| ER-05 | 🔐 User B cannot POST reaction to a plan they don't own | 403 Forbidden | P0 🔐 |
...
| PA-04 | Pantry items from Household B not visible to Household A | Isolation confirmed | P0 🔐 |

### 4.2 Activity Feed
| # | Test | Expected | Priority |
|---|------|----------|----------|
| AF-01 | Add recipe → activity feed shows "recipe_added" event | Event appears with recipe name and username | P1 |
| AF-02 | Delete recipe → "recipe_deleted" shown | Correct | P1 |
| AF-03 | Add meal to plan → "plan_meal_added" shown | Correct | P1 |
...
| AF-06 | 🔐 Activity feed only shows this household's actions | Cross-household recipe names never appear | P0 🔐 |
...
| # | Test | Expected | Priority |
|---|------|----------|----------|
| SL-01 | Plan a week with 3 recipes, generate shopping list | All ingredients from those 3 recipes appear | P0 🤖 |
...
| SL-06 | Recipe uses "garlic powder" | Listed under Pantry, not Produce | P0 🔐 (logic bug risk) |
...
| SL-11 | Pantry staple "olive oil" exists → olive oil in shopping list | Item marked as "in pantry" / greyed | P1 |
...
| SI-04 | Shopping list with 1000+ ingredients | Renders without jank (O(1) Map lookup, not O(n²)) | P2 |

---

## Section 6 — AI Features (Kitchen Copilot)

### 6.1 Copilot Flow
| # | Test | Expected | Priority |
|---|------|----------|----------|
| CP-01 | Open Copilot panel | Right drawer on desktop, bottom sheet on mobile | P0 🤖 |
| CP-02 | Complete step 1: pick meal type | Step 2 unlocks | P0 |
| CP-03 | Complete steps 1–3 (meal + cuisine + vibe) | "Search Recipes" button enabled (protein is optional) | P0 |
...
| CP-08 | Free tier — 21st copilot message today | 429 with "upgrade to premium" prompt | P1 🔐 |
| CP-09 | Premium user — no rate limit shown | Messages go through without limits | P1 |
| CP-10 | Vibes grid on mobile: 7 vibe options | No orphaned single item — uses grid-cols-2 sm:grid-cols-3 | P2 |
...
## Section 7 — Household System

### 7.1 Setup & Invites
| # | Test | Expected | Priority |
|---|------|----------|----------|
| HH-01 | New user registers → household auto-created | Household ID assigned | P0 |
...
| HH-05 | Regenerate invite link | Old link no longer valid, new 16-char code issued | P1 🔐 |
| HH-06 | 🔐 Guess a random 16-char code | 404 from preview endpoint | P1 🔐 |

### 7.2 Data Isolation
| # | Test | Expected | Priority |
|---|------|----------|----------|
| DI-01 | Household A recipes not visible to Household B | GET /api/recipes returns only householdId-scoped results | P0 🔐 |
| DI-02 | Household B cannot edit Household A's recipes | 403 or 404 | P0 🔐 |
| DI-03 | Household B plan not visible to Household A | GET /api/plans/:week returns 404 for another household's data | P0 🔐 |
| DI-04 | Activity log of Household A not in Household B feed | Zero cross-household activity leakage | P0 🔐 |
| DI-05 | Pantry of Household A not in Household B | Zero leakage | P0 🔐 |

---

## Section 8 — Profile & Onboarding
...
| # | Test | Expected | Priority |
|---|------|----------|----------|
| OB-01 | New user → redirected to `/onboarding` on first login | Onboarding flow starts | P0 |
...
| SEC-05 | Password in API response | `GET /api/user` — no `password` field in JSON | Automated (safeUser check) |
...
| # | Test | Method | Expected |
|---|------|--------|----------|
| SEC-16 | `/api/dev/upgrade-testuser` on production | POST (production NODE_ENV) | Route not registered — 404 |
...
| P-05 | Railway deploy → health check passes | 200 on first request within 30s | P0 |
| P-06 | Missing `ANTHROPIC_API_KEY` on server | AI endpoints return 503 with clear error | P1 |

---

## Section 12 — Pre-Release Smoke Checklist
...
[ ] 8.  Generate shopping list — ingredient appears under correct category
[ ] 9.  Add a pantry staple — activity feed updates
[ ] 10. Open Copilot, complete all 3 required steps, get recipe results
[ ] 11. Save a copilot recipe to the library
[ ] 12. Go to Profile — invite link is present (16 chars)
[ ] 13. Forgot password? — submit email, receive email (check inbox)
[ ] 14. Change password on Profile page
[ ] 15. Logout → can't access /api/recipes → login again with new password
```

---

## Section 13 — Known Gaps (Deferred)
...
| Fuzzy ingredient dedup in shopping list | Low | "boneless chicken breast" vs "boneless, skinless chicken breast" → two entries. Needs normalized comparison. |
...
- Expected auto-tags: crockpot (if "crockpot" in title), slow-cook (if ≥ 240 min)

---

## Appendix B — Regression Triggers

These are bugs that were fixed and must not regress. Each has an automated test — if the test breaks, the bug is back.
...
| guessCategory: "garlic cloves" → pantry (wrong) | `guess-category.test.ts: "garlic cloves → produce"` | 7c6527f |
| guessCategory: "fish sauce" → protein (wrong) | `guess-category.test.ts: "fish sauce → condiments"` | 7c6527f |
...
| Shopping list O(n²) recipe lookup | `shopping.tsx useMemo Map` — manual perf test with 500 recipes | 0c7e0b9 |
