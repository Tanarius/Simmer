# Simmer â€” Monetization Strategy
...
| Plan to Eat | Subscription | $5.95/mo or $49/yr | 14-day trial only â€” no permanent free |
...
Almost no app prices explicitly for households. AnyList is the only outlier at $14.99/yr for 2â€“4 users â€” that's barely $1.25/user/month. Every other app charges per individual account but tacitly lets households share.

Simmer is *built for households*. That's the differentiator and the pricing lever.

---

## Recommended Tier Structure

### Free Tier (generous â€” maximize top-of-funnel)

The goal of the free tier is to get a household using Simmer until it's load-bearing â€” they're planning meals, the shopping list is working, the pantry is theirs. That's when they'll pay.

| Feature | Free Limit | Rationale |
|---------|-----------|-----------|
| Recipe library | Unlimited | Pain point if capped â€” users bounce |
| URL import | Unlimited | Discovery hook; more recipes = stickier |
| Social media import (caption paste) | Unlimited | Differentiator â€” let it spread |
| Social media import (screenshot) | 3/day | Costs Sonnet credits â€” reasonable gate |
...
| Kitchen Copilot messages | **10/day** | Down from current 20 â€” enough to taste it |
...
### Premium â€” $6/month or $49/year ($4.08/mo)
...
| Nutrition data | âœ… (future â€” via Spoonacular) |
| Recipe export (PDF) | âœ… (future) |
| Public recipe pages | âœ… (future) |
| Priority support | âœ… |

**Pricing rationale:** $6/mo is below Samsung Food's $6.99, competitive with Plan to Eat's $5.95, and feels cheap for a household (3 people = $2/person/month). Annual at $49 creates retention and pays roughly $4/mo â€” strong incentive to go annual.
...
## Free â†’ Paid Conversion Strategy

### Gate the right things

The features most likely to trigger upgrades are:
1. **AI rate limits** â€” hitting "You've used your 10 messages today" mid-planning session
2. **Household member cap** â€” trying to invite a 4th person (college apartment scenario)
3. **Screenshot import** â€” people love this feature once they try it; daily limit creates urgency

These gates are frustration-at-the-right-moment. The user already believes in the product when they hit them.

### What NOT to gate

- **Recipe import / URL** â€” this is how people populate their library. Gate this and they can't get to the point of caring about premium.
- **Shopping list** â€” same reason. If the shopping list doesn't work, the core loop is broken.
- **Planner** â€” same. Let them feel the value fully before asking to pay.

### Upgrade prompt design

Current: `429 with upgradePrompt: true` â€” a flag exists, upgrade UI needs to be built.

Recommended copy: *"You've used your 10 free AI messages today. Upgrade to Premium â€” $6/mo for the whole household, unlimited AI, cancel anytime."*

Inline upgrade should open a Stripe Checkout session â€” no redirect to a separate pricing page, one step.
...
At 10,000 MAU with the same conversion: ~$24,500 ARR. These are conservative â€” meal planning apps that become habit-forming (weekly shopping list usage) have high retention. Plan to Eat reportedly has >60% annual renewal.
...
- Webhook handler: `POST /api/billing/webhook` â€” sets `subscriptionTier = 'premium'` on successful payment
- Portal endpoint: `POST /api/billing/portal` â€” Stripe Customer Portal for cancellation/upgrade
...
Put Instacart/Kroger/Walmart affiliate links on shopping list items. User clicks an item â†’ opens Instacart search pre-filled with that ingredient. Affiliate commission: ~2â€“5% of cart value.

Implementation: `getAffiliateUrl(ingredientName)` â†’ `https://www.instacart.com/products?q=${encodeURIComponent(name)}&utm_source=Simmer`. Add a small basket icon on each shopping list item for premium users.
...
This is a strong upgrade driver for the health-conscious segment â€” the subset of your users who meal prep specifically for fitness.
...
## Fair Free vs. Paid Split â€” The One-Line Answer

**Free should feel complete for a casual household. Paid should feel essential for a serious one.**

A household planning 3â€“4 dinners a week, using the copilot once or twice a week, inviting 2â€“3 people: that's free. A household doing full weekly AI planning every week, rebuilding their plan on Saturday, importing recipes from social media daily, with 5 members: that's premium. The gates should fall exactly between those two users.
...
Phase 3 (GROWTH):
  ...
  [ ] Shareable meal plan cards ("Plan with Simmer")
  [ ] Referral credit system (invite 3 â†’ 1 month free)
```
